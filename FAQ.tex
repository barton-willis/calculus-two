...
\item  \textbf{Question}  Can a function be infinity?
 ...
 \item \textbf{Question} Since $\int x^3 \, \mathrm{d}x = \frac{1}{4} x^4$,
  so it follows that $\int \cos(x)^3 \, \mathrm{d}x = \frac{1}{4} \cos(x)^4$, right?
  
\textbf{Answer} No, the formulae $\int x^3 \, \mathrm{d}x$
and $\int \cos(\theta)^3 \, \mathrm{d}\theta$ don't match.
To match the differential parts $\mathrm{d}x$ and $\mathrm{d} \theta$, we need to match $x$ with $\theta$.
But to match the integrand, we need to match $x$ with $\cos(\theta)$.
We can't match $x$ with both $\theta$ and with $\cos(\theta)$.
Maybe you are thinking of  $\int \cos(x)^3 \, \mathrm{d} \cos(x)$.  This matches with
$\int x^3 \, \mathrm{d}x$. So $\int \cos(x)^3 \, \mathrm{d} \cos(x) = 
\frac{1}{4} \cos(x)^4$ is correct.
