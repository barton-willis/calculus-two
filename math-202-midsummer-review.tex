...
\begin{parts}
    ...
    \begin{equation}
      ...
      \end{equation}
    Some teachers will insist on the $+c$.  I say let's just  remember that all anti-derivatives are undetermined 
    up to an additive constant and implicitly include the additive constant in with the left side of
      \begin{equation}
      \int x^2 - x - 2 \, \mathrm{d} x = \frac{1}{3} x^3 - \frac{1}{2} x^2 - 2 x
      \end{equation}    
    (and not the right side) and for forget the silly explicit $+c$ rule. It is maybe a bit peculiar, but we do need to remember that
    \begin{equation}
      \int F(x) \, \mathrm{d} x =  \int F(x) \, \mathrm{d} x
     \end{equation}
     is \emph{not} an identity. Usually visually equal things are equivalent, but this case is an exception.
